...
VideoPlayerTypes g_video_player_type;

QString g_vlc_url;
int g_vlc_port;
QString g_vlc_password;
bool g_is_video_player_time_in_milliseconds = false;

QString g_rvp_url;
int g_rvp_port;

QString g_HereSphere_ip_address;
int g_HereSphere_port;

QString g_hotkey_stop;
QString g_hotkey_pause;
QString g_hotkey_resume;
QString g_hotkey_use_modify_funscript_functions;
QString g_hotkey_funscript_time_shift_up;
QString g_hotkey_funscript_time_shift_down;
...
bool g_pause = false;
std::atomic<bool> g_update{ false };
std::atomic<bool> g_msg_created{ false };
...
ThreadedPlayerStatus g_threaded_player_status;
VideoPlayerStatus g_video_player_status;
...
// Dynamic accessor for funscript_data_maped[i].first that reflects the
// current value of g_funscript_time_shift_ms on every invocation.
// funscript_data_maped[i].first itself is NEVER modified (kept pristine).
#define FS_FIRST(i) (funscript_data_maped[i].first + (int)g_funscript_time_shift_ms.load(std::memory_order_relaxed))
...
void get_new_camera_frame(cv::VideoCapture &capture, cv::Mat& frame, __int64& msec_frame_cur_pos)
{
	...
	static LARGE_INTEGER frame_read_cur_time;
	static bool res = false;

	if (g_threaded_capture.is_running)
	{
		res = g_threaded_capture.wait_and_get_fresh_frame(frame, msec_frame_cur_pos, frame_read_cur_time);
	}
	else
	{
		do
		{
			...
			QueryPerformanceCounter(&frame_read_cur_time);
	...
	if (res)
	{
		get_delta_frame_read_time_vs_video_time(msec_frame_cur_pos, frame_read_cur_time);
	}
}
...
void find_objects(const cv::Mat& mask, std::vector<ObjectData>& result, cv::Mat& labels)
{
	cv::Mat stats, centroids;

	// Call the optimized C++ algorithm for marking connected components
	// connectivity=8 takes into account diagonal touching of points
	int num_labels = cv::connectedComponentsWithStats(mask, labels, stats, centroids, 8, CV_32S);

	result.resize(num_labels - 1);

	// Index 0 is always a black background, we completely ignore it (start with i = 1)
	for (int i = 1; i < num_labels; ++i)
	{
		ObjectData &obj = result[i - 1];
		obj.m_id = i;
		obj.p_labels = &labels;
		obj.m_size = stats.at<int>(i, cv::CC_STAT_AREA);
		obj.m_minX = stats.at<int>(i, cv::CC_STAT_LEFT);
		obj.m_minY = stats.at<int>(i, cv::CC_STAT_TOP);
		obj.width = stats.at<int>(i, cv::CC_STAT_WIDTH);
		obj.height = stats.at<int>(i, cv::CC_STAT_HEIGHT);
	}
}
...
bool get_hismith_pos_by_image(cv::Mat& frame, int& pos, bool ignore_error = false, bool show_results = false, cv::Mat *p_res_frame = NULL, double *p_cur_speed = NULL, cv::String title = "Get Hismith Pos By Image", QString add_data = QString())
{
	...
	bool res = true;
	cv::Mat img, img_b, img_g, labels_b, labels_g, img_right;
	std::vector<ObjectData> figures_b;
	std::vector<ObjectData*> p_figures_b;
	int figures_b_N;
	std::vector<ObjectData> figures_g;
	...
	concurrency::parallel_invoke(
		[&img_b, &img_g, &figures_b, &labels_b, &p_figures_b, &figures_b_N, width, height] {
			img_b = img_b & (~img_g);
			find_objects(img_b, figures_b, labels_b);

			figures_b_N = figures_b.size();

			if (figures_b_N > 0)
			{
				p_figures_b.resize(figures_b_N);
		...
		[&img_g, &figures_g, &labels_g, width, height] {
			find_objects(img_g, figures_g, labels_g);
		}
	);
	...
	if (p_best_match_l_figure)
	{
		...
		int size = p_best_match_l_figure->m_size;
		int w = p_best_match_l_figure->width, h = p_best_match_l_figure->height;

		cv::Rect obj_roi(p_best_match_l_figure->m_minX, p_best_match_l_figure->m_minY, w, h);
		cv::Mat labels_roi = (*p_best_match_l_figure->p_labels)(obj_roi);
		cv::Mat obj_mask = (labels_roi == p_best_match_l_figure->m_id);
		uchar* p_mask_data = obj_mask.data;

		if (!obj_mask.isContinuous())
		{
			error_msg("ERROR: obj_mask.isContinuous() == false");
	...
	else
	{
		...
		for (int id1 = 0; id1 < figures_b_N - 1; id1++)
		{
			for (int id2 = id1 + 1; id2 < figures_b_N; id2++)
			{
				...
				{
					ObjectData* pFigure = p_figures_b[id1];
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						cv::Rect obj_roi(p_best_match_c_figure->m_minX, p_best_match_c_figure->m_minY, p_best_match_c_figure->width, p_best_match_c_figure->height);
						cv::Mat labels_roi = (*p_best_match_c_figure->p_labels)(obj_roi);
						cv::Mat obj_mask = (labels_roi == p_best_match_c_figure->m_id);
						uchar* p_mask_data = obj_mask.data;
						...
						{
							for (l = 0; l < p_best_match_c_figure->width * p_best_match_c_figure->height; l++)
							{
								if (p_mask_data[l])
								{
									// getting c_figure point position
									x = p_best_match_c_figure->m_minX + (l % p_best_match_c_figure->width);
									y = p_best_match_c_figure->m_minY + (l / p_best_match_c_figure->width);
					...
					if (res)
					{
							...
							(g_ccxlcx_lh_ratio > g_max_telescopic_motor_rocker_arm_center_x_proportions))
						{
							if (!ignore_error)
							{
								...
								img_res.setTo(cv::Scalar(255, 0, 0), *p_best_match_l_figure->p_labels == p_best_match_l_figure->m_id);
								img_res.setTo(cv::Scalar(255, 255, 0), *p_best_match_r_figure->p_labels == p_best_match_r_figure->m_id);
								img_res.setTo(cv::Scalar(255, 0, 255), *p_best_match_c_figure->p_labels == p_best_match_c_figure->m_id);
				...
				if (res)
				{
					for (int id = 0; id < figures_g.size(); id++)
					{
						ObjectData* pFigure = &(figures_g[id]);
						int x = pFigure->m_minX, y = pFigure->m_minY, w = pFigure->width, h = pFigure->height;
						int size = pFigure->m_size;
					...
					else
					{
						...
						else
						{
							...
							if (show_results)
							{
								...
								img_res.setTo(cv::Scalar(255, 0, 0), *p_best_match_l_figure->p_labels == p_best_match_l_figure->m_id);
								img_res.setTo(cv::Scalar(255, 0, 0), *p_best_match_r_figure->p_labels == p_best_match_r_figure->m_id);
								img_res.setTo(cv::Scalar(255, 0, 0), *p_best_match_c_figure->p_labels == p_best_match_c_figure->m_id);
								img_res.setTo(cv::Scalar(0, 255, 0), *p_best_match_g_figure->p_labels == p_best_match_g_figure->m_id);
...
void ThreadedCapture::capture_loop() {
	...
	while (is_running)
	{
		...
		{
			QueryPerformanceCounter(&frame_read_time);
...
void set_camera_settings(cv::VideoCapture& capture)
{
	...
	if (g_webcam_exposure < 0)
	{
		...
		for (int i = 0; i < g_webcam_fps / 2; i++)
		{
			capture.read(frame);
		}
...
bool init_camera(cv::VideoCapture &capture)
{
	...
	if (res)
	{
		...
		// geting first 5 * g_webcam_fps (~5 seconds) frames for get better camera focus and correct g_delta_frame_read_time_vs_video_time
		for (int i = 0; i < 5 * g_webcam_fps; i++)
		{
...
void make_rvp_status_request(QNetworkAccessManager* manager, QNetworkRequest* req, bool& is_paused, QString& video_filepath, int& video_pos, __int64& sys_time, double& rate)
{
	bool res = false;
	video_pos = -1;
	sys_time = -1;
	...
	int length;
	double position;
	bool show_warning_rvp_is_not_started = true;
	bool show_warning_no_video_selected = true;
	QString sync_data;
	QString ReqUrl(g_rvp_url + ":" + QString::number(g_rvp_port) + "/variables.html");
	...
	do
	{
		...
		if (rep->isFinished() == true)
		{
			...
			{
				reply_res = QString::fromUtf8(rep->readAll());
				res = true;
			}
		}
		...
		{
		   /*
		   <p id="file">%file_name%</p>
		   <p id="filepatharg">...</p>
		   <p id="filepath">%file_path%</p>
		   <p id="filedir">...</p>
		   <p id="state">1</p>
		   <p id="position">%number%</p>
		   ...
		   */
			...
			QRegularExpressionMatchIterator it = regex.globalMatch(reply_res);
			...
			while (it.hasNext()) {
				...
				if (key == "filepath")
				{
					video_filepath = QDir::toNativeSeparators(value);
					found++;
				}
				else if (key == "state")
				{
			...
			if (res)
			{
				if (video_filepath.length() == 0)
				{
					if (show_warning_no_video_selected)
					{
						show_msg("Video file is not opened");
						show_warning_no_video_selected = false;
						show_warning_rvp_is_not_started = true;
					}
					res = false;
				}
			}
...
void make_rvp_command(QNetworkAccessManager* manager, VideoPlayerCommand command)
{
	QString ReqUrl(g_rvp_url + ":" + QString::number(g_rvp_port) + "/command.html");
	...
	{
		postData.append("wm_command=888");
	}
	else
	{
		return;
	...
	/*if (!rep->isFinished())
	{
		error_msg("ERROR: failed to make make_rvp_command");
	}*/
}
...
void make_here_sphere_status_request(bool& is_paused, QString& video_filepath, int& video_pos, __int64& sys_time, double& rate)
{
	...
	do
	{
		...
		if (!data.gotData)
		{
			if (show_warning_here_sphere_is_not_started && (dt >= 2000))
			{
				show_msg(
					"Unable to receive information from Here Sphere for the last 2 seconds:\n"
					"Waiting for Here Sphere to start sending data.");
				show_warning_here_sphere_is_not_started = false;
				show_warning_no_video_selected = true;
			}
		}
		else
		{
			if (data.videoFilePath.size() == 0)
			{
				if (show_warning_no_video_selected)
				{
					show_msg("Video file is not selected: waiting for video file selection");
					show_warning_no_video_selected = false;
					show_warning_here_sphere_is_not_started = true;
				}
			}
			else
			{
				video_filepath = QDir::toNativeSeparators(data.videoFilePath);
				is_paused = data.isPaused;
				rate = data.videoRate;

				if (is_paused)
				{
					video_pos = (data.videoPos * 1000.0);
				}
				else
				{
					...
					QueryPerformanceFrequency(&Frequency);
					QueryPerformanceCounter(&cur_time);
					video_pos = (data.videoPos * 1000.0) + ((double)time_diff_in_milliseconds(cur_time, data.cur_time, Frequency) * rate);
				}
...
void get_cur_video_pos(const VideoPlayerStatus& st, LARGE_INTEGER &cur_time, int &cur_video_pos, bool show_waring = true)
{
	if ((st.sys_time > 0) && !st.is_paused)
	{
		struct _timeb timebuffer;
		_ftime(&timebuffer);
		QueryPerformanceCounter(&cur_time);
		__int64 cur_sys_time = (((__int64)timebuffer.time) * 1000) + timebuffer.millitm;
		__int64 d_time = cur_sys_time - st.sys_time;
		...
		cur_video_pos = st.video_pos + (int)((double)d_time * st.rate);
	}
	else
	{
		cur_video_pos = st.video_pos;
		QueryPerformanceCounter(&cur_time);
	}
}
...
void show_msg(QString msg, int timeout, MessageType msg_type, bool drow_modify_funscript_functions, double msg_wnd_y_offset_from_top)
{
	...
	if (timeout > 0)
	{
		...
		_tmp_create_msg_cvar.wait(lk, [] { return g_msg_created.load(); });
		int j = 5;
	}
}
...
{
	int dt = (double)(g_video_cur_actions_end_time - g_actual_video_pos) / g_video_player_status.rate;
	int min, sec;
	sec = (int)(dt / 1000);
	min = sec / 60;
	sec = sec % 60;
...
VideoPlayerStatus make_video_player_status(VideoPlayerCommand command = VideoPlayerCommand::None)
{
	...
	if (g_video_player_type == VideoPlayerTypes::VLC)
	{
		static QString _tmp_video_filename;
		static QString _tmp_video_filepath;
		...
		if (video_filename != _tmp_video_filename)
		{
			...
			while (!n.isNull()) {
				...
				if (!e.isNull()) {
					...
					if (tag_name == "leaf")
					{
						...
						if (current == "current")
						{
							_tmp_video_filepath = QDir::toNativeSeparators(QUrl(uri).toLocalFile());
							break;
						}
					}
				}
			...
			if (_tmp_video_filepath.length() == 0)
			{
				error_msg(QString("ERROR: Failed to get full video file path for video with name: %1").arg(video_filename));
			}

			result.video_filepath = _tmp_video_filepath;
		}
		else
		{
			result.video_filepath = _tmp_video_filepath;
	...
	if (result.video_filepath.size() > 0 && result.rate <= 0)
	{
...
void ThreadedPlayerStatus::start(VideoPlayerStatus& initial_state, unsigned int poll_interval_ms)
{
	stop();
	...
	else
	{
		...
		{
			m_cached_state = VideoPlayerStatus();
			QueryPerformanceCounter(&m_cached_state.poll_time);
			initial_state = m_cached_state;
		}
	}
	...
	m_is_running = true;
	m_worker_thread = std::thread(&ThreadedPlayerStatus::status_loop, this);
}

void ThreadedPlayerStatus::stop()
{
	if (m_is_running)
	{
		{
...
VideoPlayerStatus ThreadedPlayerStatus::make_video_player_status(VideoPlayerCommand command)
{
...
VideoPlayerStatus ThreadedPlayerStatus::getLatestState() const
{
	std::lock_guard<std::mutex> lk(m_state_mutex);
	return m_cached_state;
}
...
void ThreadedPlayerStatus::status_loop()
{
	while (!m_stop_requested)
	{
...

		{
			...
			m_stop_cv.wait_for(lock, std::chrono::milliseconds(m_poll_interval_ms), [this] {
...
void run_funscript()
{
	...
	LARGE_INTEGER prev_vp_st_poll_time;
	QueryPerformanceFrequency(&Frequency);
	...
	g_results_file_data += QString(
		"video_player_type:%1\n"
		"is_video_player_time_in_milliseconds:%2\n"
		"video_player_sys_time:%3\n")
		.arg(VideoPlayerTypesUtils::to_string(g_video_player_type))
		.arg(g_is_video_player_time_in_milliseconds)
		.arg(g_video_player_status.sys_time);
		...
		while (!g_stop_run)
		{
			...
			if (g_pause && !g_update)
			{
				do
				{
					...
					get_cur_video_pos(g_video_player_status, cur_time, cur_video_pos, false);
					g_actual_video_pos = cur_video_pos;
					if (prev_rate != g_video_player_status.rate)
					{
						prev_rate = g_video_player_status.rate;
						g_update = true;
					}
			...
			last_play_video_filepath = g_video_player_status.video_filepath;
			...
			if (last_load_funscript_video_filepath != last_play_video_filepath)
			{
				...
				if (!((funscript_fname.size() > 0) && QFile::exists(funscript_fname)))
				{
					...
					do
					{
						...
						get_cur_video_pos(g_video_player_status, cur_time, cur_video_pos);
						g_actual_video_pos = cur_video_pos;
					} while (last_play_video_filepath == g_video_player_status.video_filepath && !g_stop_run);
					continue;
				}
			}
			...
			if ( (last_load_funscript_fname != funscript_fname) || g_was_change_in_use_modify_funscript_functions )
			{
				...
				last_load_funscript_video_filepath = last_play_video_filepath;
			...
			if (funscript_data_maped.size() < 2)
			{
				...
				do
				{
					...
					get_cur_video_pos(g_video_player_status, cur_time, cur_video_pos);
					g_actual_video_pos = cur_video_pos;
				} while ((last_play_video_filepath == g_video_player_status.video_filepath) && (cur_video_pos >= search_video_pos) && (cur_video_pos <= search_video_pos + 1000) && !g_stop_run);
				continue;
			}
			else
			{
				if ((int)((double)(FS_FIRST(0) - search_video_pos) / g_video_player_status.rate) > 10000)
				{
					show_msg(QString("The first funscript video action will be afte %1 seconds at pos: %2").arg((int)((double)(FS_FIRST(0) - search_video_pos)/(g_video_player_status.rate*1000.0))).arg(VideoTimeToStr(FS_FIRST(0)).c_str()), 5000, MessageType::Always);
				}
			}

			{
				...
				{
					if ((FS_FIRST(i) - FS_FIRST(i - 1)) < 10000)
					{
						last_i = i;
					}
					else
				...
				g_video_cur_actions_end_time = FS_FIRST(last_i);
			...
			uint64_t pool_size_bytes = min(static_cast<uint64_t>(g_webcam_fps *
				((double)(FS_FIRST(actions_size - 1) - cur_video_pos) / (g_video_player_status.rate * 1000.0)) *
			...
			QString start_video_name = g_video_player_status.video_filepath;
			//waiting for video unpaused

			bool position_was_aligned = false;
			bool video_was_paused = g_video_player_status.is_paused;

			if (g_is_video_player_time_in_milliseconds && g_video_player_status.is_paused)
			{
				get_new_camera_frame(*g_pCapture, frame, msec_video_cur_pos);
				get_res = get_hismith_pos_by_image(frame, cur_pos);
				if (!get_res)
				...
				if ((g_d_from_search_start_pos < g_min_search_pos_dif) || (g_d_from_search_start_pos > g_max_search_pos_dif))
				{
					...
					if (dpos < 0) dpos += 360;
					dt = (int)((double)(FS_FIRST(0) - cur_video_pos) / g_video_player_status.rate) - g_speed_change_delay;
					...
					do
					{
						...
						if (cur_speed > 0)
						{
							__int64 webcam_time_diff = g_webcam_end_to_end_latency + max((cur_time.QuadPart * (__int64)1000) / Frequency.QuadPart - (g_delta_frame_read_time_vs_video_time + msec_video_cur_pos), 0);

							if (webcam_time_diff > 0)
							{
						...
						if (g_stop_run || g_pause || g_video_freezed || !g_video_player_status.is_paused || g_was_change_in_use_modify_funscript_functions || (last_play_video_filepath != g_video_player_status.video_filepath) || (cur_video_pos > FS_FIRST(1)) || (cur_video_pos < search_video_pos) || (prev_rate != g_video_player_status.rate))
						{
							break;
						}
					...
					if (g_stop_run || g_pause || g_video_freezed || g_was_change_in_use_modify_funscript_functions || (last_play_video_filepath != g_video_player_status.video_filepath) || (cur_video_pos > FS_FIRST(1)) || (cur_video_pos < search_video_pos) || (prev_rate != g_video_player_status.rate))
					{
						if (g_pause)
						{
							g_video_player_status = g_threaded_player_status.make_video_player_status();
							if (!g_video_player_status.is_paused && (g_video_player_status.video_filepath.size() > 0))
							{
			...
			if (g_video_player_status.is_paused)
			{
				get_new_camera_frame(*g_pCapture, frame, msec_video_cur_pos);
				get_res = get_hismith_pos_by_image(frame, cur_pos);
				if (!get_res)
			...
			while (
				g_video_player_status.is_paused ||
				( ((int)((double)(FS_FIRST(0) - cur_video_pos) / g_video_player_status.rate) > g_speed_change_delay + g_min_dt_between_speed_changes_on_fast_moves) && position_was_aligned ) ||
				((int)((double)(FS_FIRST(0) - cur_video_pos) / g_video_player_status.rate) >= g_speed_change_delay + 2*g_min_dt_between_speed_changes_on_fast_moves)
				)
				{
					g_video_player_status = g_threaded_player_status.make_video_player_status();
					get_cur_video_pos(g_video_player_status, cur_time, cur_video_pos);
				...
				if (g_stop_run || g_pause || g_video_freezed || g_was_change_in_use_modify_funscript_functions || (last_play_video_filepath != g_video_player_status.video_filepath) || ((int)((double)(cur_video_pos - FS_FIRST(1)) / g_video_player_status.rate) > 200) || (cur_video_pos < search_video_pos) || (prev_rate != g_video_player_status.rate) ||
			...
			if ( (g_video_player_status.is_paused && g_update) ||
				(g_is_video_player_time_in_milliseconds && !video_was_paused && g_video_player_status.is_paused) )
			{
				continue;
			}
			...
			if (!g_is_video_player_time_in_milliseconds)
			{
			...
			while ( (action_id < actions_size) &&
					( (double)(funscript_data_maped[action_id].second - abs_cur_pos) / (double)(FS_FIRST(action_id) - cur_video_pos)
					> 1.2 * ( ((double)(funscript_data_maped[action_id].second - funscript_data_maped[action_id_start].second)) /
				  			(double)(FS_FIRST(action_id) - FS_FIRST(action_id_start)) ) )
					)
			{
				start_info += QString("\nreq_speed/actions_speed:%1 > 1.2 with action_id:%4 => action_id++")
					.arg(((double)(funscript_data_maped[action_id].second - abs_cur_pos) / (double)(FS_FIRST(action_id) - cur_video_pos)) /
						((((double)(funscript_data_maped[action_id].second - funscript_data_maped[action_id_start].second)) / (double)(FS_FIRST(action_id) - FS_FIRST(action_id_start)))))
					.arg(action_id);
				action_id++;
			}
			...
			avg_req_speed = (((double)(funscript_data_maped[action_id].second - abs_cur_pos) * 1000.0) /
				((double)(FS_FIRST(action_id) - cur_video_pos) / g_video_player_status.rate));
			...
			while (FS_FIRST(action_id) - cur_video_pos < req_dt)
			{
				start_info += QString("\n(action_id_end_time - cur_video_pos):%1 < req_dt:%2 (for avg_req_speed: %3) => action_id++")
					.arg(FS_FIRST(action_id) - cur_video_pos)
					.arg(req_dt)
			...
			start_info += QString("\n\nInitial target action_id:%1 action_id_start:%2\n"
				...
				.arg((start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate)) - FS_FIRST(0))
				.arg(abs_cur_pos - funscript_data_maped[action_id_start].second)
				.arg((start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate)) - FS_FIRST(action_id_start))
				.arg(( (double)(funscript_data_maped[action_id].second - abs_cur_pos) / (double)(FS_FIRST(action_id) - cur_video_pos) ) /
						( (((double)(funscript_data_maped[action_id].second - funscript_data_maped[action_id_start].second)) / (double)(FS_FIRST(action_id) - FS_FIRST(action_id_start))) ));
			...
			{
				__int64 frame_time_in_ms = g_delta_frame_read_time_vs_video_time + msec_video_cur_pos - g_webcam_end_to_end_latency;
				int frame_video_pos = start_video_pos + (int)((double)(frame_time_in_ms - start_time_in_ms) * g_video_player_status.rate);
				frames_data_history.push_back({ abs_cur_pos, frame_video_pos });
			}

			g_threaded_player_status.start(g_video_player_status, 250);
			...
			while (action_id < actions_size)
			{
				...
				if (cur_speed > 0)
				{
					...
					if (webcam_time_diff > 0)
					{
						int exp_abs_cur_dif = ((cur_speed * (double)(webcam_time_diff)) / 1000.0);
						exp_abs_cur_pos = abs_cur_pos + exp_abs_cur_dif;
						hismith_speed_changed += QString("\n\t[exp_abs_cur_dif:%1 webcam_time_diff:%2 cur_speed:%3]")
							.arg(exp_abs_cur_dif)
							.arg(webcam_time_diff)
							.arg(cur_speed);
					}
				}
				...
				g_actual_video_pos = start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate);
				while (actual_action_id > 1 && (FS_FIRST(actual_action_id - 1) > g_actual_video_pos))
				{
					actual_action_id--;
				}
				...
				while ( fut_action_id < actions_size - 1 &&
						(FS_FIRST(fut_action_id) - FS_FIRST(actual_action_id - 1) <= 333) )
				{
					fut_action_id++;
				}

				avg_req_speed = ( ((double)(funscript_data_maped[fut_action_id].second - funscript_data_maped[actual_action_id - 1].second) * 1000.0) /
										((double)(FS_FIRST(fut_action_id) - FS_FIRST(actual_action_id - 1)) / g_video_player_status.rate) );
				...
				dif_cur_vs_req_action_start_time = (start_video_pos + (int)((double)(time_diff_in_milliseconds(action_start_time, start_time, Frequency)) * g_video_player_status.rate)) - FS_FIRST(action_id - 1);

				dt = ((double)(FS_FIRST(action_id) - (start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate))) / g_video_player_status.rate);
				if (dt < g_speed_change_delay + min_dt_between_speed_changes)
				{
					...
					results[action_id - 1].action_start_video_time = VideoTimeToStr(FS_FIRST(action_id - 1)).c_str();
					results[action_id - 1].dif_cur_vs_req_action_end_time = (start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate)) - FS_FIRST(action_id);
					results[action_id - 1].dif_cur_vs_req_action_start_time = dif_cur_vs_req_action_start_time;
					results[action_id - 1].action_length_time = FS_FIRST(action_id) - FS_FIRST(action_id - 1);
					...
					((cur_set_hismith_speed == 0) && ((int)(time_diff_in_milliseconds(cur_time, set_hismith_speed_time, Frequency)) > 3*min_dt_between_speed_changes)) )
				{
					...
					hismith_speed_changed += QString("\n\t[act_start spd_change: new_set_h_spd:%1 opt_h_spd:%2 set_h_spd_dt:%3 tm_ofs_to_end:%4 cur_spd:%5 req_spd:%6 req_pos_vs_cur:%7]")
						...
						.arg(FS_FIRST(action_id) - (start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate)))
						.arg(cur_speed)
						.arg(req_speed)
				...
				do
				{
					...
					if (g_video_player_status.poll_time.QuadPart != prev_vp_st_poll_time.QuadPart)
					{
						...
						LARGE_INTEGER vp_st_poll_time = g_video_player_status.poll_time;
						...
						if (g_is_video_player_time_in_milliseconds)
						{
							if (g_video_player_status.sys_time > 0)
							{
								new_start_video_pos = cur_video_pos - (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate);
								start_video_pos_dif = new_start_video_pos - start_video_pos;
								start_video_pos = new_start_video_pos;
								hismith_speed_changed += QString("\n\t[get_video_player_status: start_video_pos_dif:%1 new_start_video_pos:%2]").arg(start_video_pos_dif).arg(new_start_video_pos);
							}
							else
							{
								new_start_video_pos = cur_video_pos - (int)((double)(time_diff_in_milliseconds(vp_st_poll_time, start_time, Frequency)) * g_video_player_status.rate);
								start_video_pos_dif = new_start_video_pos - start_video_pos;
								if (start_video_pos_dif > 0)
								{
									start_video_pos = new_start_video_pos;
									hismith_speed_changed += QString("\n\t[get_video_player_status: start_video_pos_dif:%1 new_start_video_pos:%2]").arg(start_video_pos_dif).arg(new_start_video_pos);
								}
					...
					{
						double dt2 = (double)(FS_FIRST(action_id + 1) - (start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate))) / g_video_player_status.rate;
				...
				if (g_video_player_status.poll_time.QuadPart != prev_vp_st_poll_time.QuadPart)
				{
					...
					else if (g_is_video_player_time_in_milliseconds)
					{
						if (g_video_player_status.sys_time > 0)
						{
							new_start_video_pos = cur_video_pos - (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate);
				...
				if (g_stop_run || g_pause || g_video_freezed || g_was_change_in_use_modify_funscript_functions || g_video_player_status.is_paused ||
					((int)((double)(cur_video_pos - (start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate))) / g_video_player_status.rate) > (g_is_video_player_time_in_milliseconds ? 300 : 1000)) ||
					(cur_video_pos < prev_cur_video_pos - 300) || (last_play_video_filepath != g_video_player_status.video_filepath) || (prev_rate != g_video_player_status.rate))
				{
					if (last_play_video_filepath != g_video_player_status.video_filepath)
					{
						show_msg("Played video was changed");
						actions_end_with = QString("last_play_video_filepath (%1) != video_filepath (%2)").arg(last_play_video_filepath).arg(g_video_player_status.video_filepath);
					}
					else if ((int)((double)(cur_video_pos - (start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate))) / g_video_player_status.rate) > (g_is_video_player_time_in_milliseconds ? 300 : 1000))
					{
						show_msg("Video time was jumped forward");
						actions_end_with = QString("(cur_video_pos - (start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate))) (%1)  > %2")
							.arg(cur_video_pos - (start_video_pos + (int)((double)(time_diff_in_milliseconds(cur_time, start_time, Frequency)) * g_video_player_status.rate)))
							.arg(g_is_video_player_time_in_milliseconds ? 300 : 1000);
					}
					...
					else if (prev_rate != g_video_player_status.rate)
					{
						show_msg(QString("Video speed rate was changed to: %1").arg(g_video_player_status.rate));
						actions_end_with = QString("prev_rate: %1 != g_video_player_status.rate: %2")
							.arg(prev_rate)
							.arg(g_video_player_status.rate);
					}
					else if (g_pause)
					...
					else if (g_video_player_status.is_paused)
					{
						actions_end_with = QString("video_is_paused");
					}
					else
					{
			...
			if (actions_size > 2)
			{
				...
				for (const FrameData& frame_data : frames_data_history)
				{
					...
					if (frame_data.video_pos >= FS_FIRST(_id))
					{
						if (frame_data_prev.video_pos <= FS_FIRST(_id))
						{
							double exp_abs_cur_pos_to_req_time =
								(double)frame_data_prev.abs_pos +
								(((double)(FS_FIRST(_id) - frame_data_prev.video_pos) /
	...
	g_threaded_player_status.stop();
	g_video_player_status = g_threaded_player_status.make_video_player_status();
	if ((g_video_player_status.video_filepath.size() != 0) && (!g_video_player_status.is_paused))
	{
		g_video_player_status = g_threaded_player_status.make_video_player_status(VideoPlayerCommand::Pause);
	}
...
void get_performance_with_hismith(int hismith_speed)
{
	...
	if (init_camera(capture))
	{
		...
		while (1)
		{
			...
			max_webcam_time_diff = max((cur_time.QuadPart * (__int64)1000) / Frequency.QuadPart - (g_delta_frame_read_time_vs_video_time + msec_video_cur_pos), max_webcam_time_diff);
...
void xmlToLinearList(const QDomElement& element, const QString& currentPath, QList<QPair<QString, QString>>& dataList) {
	...
	while (!n.isNull()) {
		...
		if (!e.isNull()) {
			...
			QString newPath = currentPath.isEmpty() ? e.tagName() : currentPath + "/" + e.tagName();
			xmlToLinearList(e, newPath, dataList);
		}
		n = n.nextSibling();
	}

	if (!hasChildElements && !currentPath.isEmpty()) {
		dataList.append({ currentPath, element.text().trimmed() });
	}
}

void linearListToXml(QDomDocument& doc, QDomElement& root, const QList<QPair<QString, QString>>& dataList) {
	for (const auto& pair : dataList) {
		QString key = pair.first;
		QString value = pair.second;

		QStringList parts = key.split('/');
		QDomElement currentElem = root;

		for (int i = 0; i < parts.size(); ++i) {
			QString part = parts[i];

			QDomElement child = currentElem.firstChildElement(part);
			if (child.isNull()) {
				child = doc.createElement(part);
				currentElem.appendChild(child);
			}

			currentElem = child;

			if (i == parts.size() - 1) {
				while (currentElem.hasChildNodes()) {
					currentElem.removeChild(currentElem.firstChild());
				}
				currentElem.appendChild(doc.createTextNode(value));
			}
		}
	}
}
...
void SaveSettings()
{
	...
	QList<QPair<QString, QString>> data_list;

	data_list.append({ "max_allowed_hismith_speed", QString::number(g_max_allowed_hismith_speed) });
	data_list.append({ "hismith_speed_for_set_initial_pos", QString::number(g_hismith_speed_for_set_initial_pos) });
	data_list.append({ "min_funscript_relative_move", QString::number(g_min_funscript_relative_move) });
	data_list.append({ "use_modify_funscript_functions", QString::number(g_modify_funscript ? 1 : 0) });
	data_list.append({ "functions_move_variants", g_modify_funscript_function_move_variants });
	data_list.append({ "functions_move_in_out_variants", g_modify_funscript_function_move_in_out_variants });
	data_list.append({ "functions_move_in_out_variant", QString::number(g_functions_move_in_out_variant) });
	data_list.append({ "dt_for_get_cur_speed", QString::number(g_dt_for_get_cur_speed) });
	data_list.append({ "min_dt_between_speed_changes_on_slow_moves", QString::number(g_min_dt_between_speed_changes_on_slow_moves) });
	...
	data_list.append({ "G_range", QString("[%1-%2][%3-%4][%5-%6]")
												.arg(g_G_range[0][0])
												.arg(g_G_range[0][1])
												.arg(g_G_range[1][0])
												.arg(g_G_range[1][1])
	...
	data_list.append({ "max_telescopic_motor_rocker_arm_proportions", QString::number(g_max_telescopic_motor_rocker_arm_proportions) });
	data_list.append({ "min_telescopic_motor_rocker_arm_center_x_proportions", QString::number(g_min_telescopic_motor_rocker_arm_center_x_proportions) });
	data_list.append({ "max_telescopic_motor_rocker_arm_center_x_proportions", QString::number(g_max_telescopic_motor_rocker_arm_center_x_proportions) });

	data_list.append({ "req_webcam_name", selected_webcam });
	data_list.append({ "webcam_frame_width", QString::number(g_webcam_frame_width) });
	data_list.append({ "webcam_frame_height", QString::number(g_webcam_frame_height) });
	data_list.append({ "webcam_fps", QString::number(g_webcam_fps) });
	data_list.append({ "webcam_focus", QString::number(g_webcam_focus) });
	data_list.append({ "webcam_exposure", QString::number(g_webcam_exposure) });
	data_list.append({ "webcam_end_to_end_latency", QString::number(g_webcam_end_to_end_latency) });

	data_list.append({ "intiface_central_client_url", QString(g_intiface_central_client_url.c_str()) });
	data_list.append({ "intiface_central_client_port", QString::number(g_intiface_central_client_port) });
	data_list.append({ "hismith_device_name", selected_device });

	data_list.append({ "video_player_types/VLC/url", g_vlc_url });
	data_list.append({ "video_player_types/VLC/port", QString::number(g_vlc_port) });
	data_list.append({ "video_player_types/VLC/password", g_vlc_password });

	data_list.append({ "video_player_types/RVP/url", g_rvp_url });
	data_list.append({ "video_player_types/RVP/port", QString::number(g_rvp_port) });

	data_list.append({ "video_player_types/HereSphere/ip_address", g_HereSphere_ip_address });
	data_list.append({ "video_player_types/HereSphere/port", QString::number(g_HereSphere_port) });
	...
	data_list.append({ "hotkey_resume", g_hotkey_resume });
	data_list.append({ "hotkey_use_modify_funscript_functions", g_hotkey_use_modify_funscript_functions });
	data_list.append({ "hotkey_funscript_time_shift_up", g_hotkey_funscript_time_shift_up });
	data_list.append({ "hotkey_funscript_time_shift_down", g_hotkey_funscript_time_shift_down });

	linearListToXml(document, root, data_list);
...
bool LoadSettings()
{
	...
	QList<QPair<QString, QString>> data_list;
	QDomElement docElem = doc.documentElement();
	xmlToLinearList(docElem, "", data_list);
	QHash<QString, QString> data_map(data_list.begin(), data_list.end());
	...
	g_hotkey_funscript_time_shift_up = data_map["hotkey_funscript_time_shift_up"];
	g_hotkey_funscript_time_shift_down = data_map["hotkey_funscript_time_shift_down"];
	...
	g_video_player_type = VideoPlayerTypesUtils::from_string(data_map["video_player_type"]);

	g_vlc_url = data_map["video_player_types/VLC/url"];
	g_vlc_port = data_map["video_player_types/VLC/port"].toInt();
	g_vlc_password = data_map["video_player_types/VLC/password"];

	g_rvp_url = data_map["video_player_types/RVP/url"];
	g_rvp_port = data_map["video_player_types/RVP/port"].toInt();

	g_HereSphere_ip_address = data_map["video_player_types/HereSphere/ip_address"];
	g_HereSphere_port = data_map["video_player_types/HereSphere/port"].toInt();
	...
	//--------------------
	{
		...
		QString video_player_type_str = VideoPlayerTypesUtils::to_string(g_video_player_type);
		int index = g_pW->ui->videoPlayerType->findText(video_player_type_str);

		if (index != -1)
		{
			g_pW->ui->videoPlayerType->setCurrentIndex(index);
		}
		else
		{
			error_msg(QString("ERROR: Failed to find video player type in list: %1").arg(video_player_type_str));
		}
	}
	//--------------------
...
void test_vlc()
{
	...
	double cur_rate = 1;
	bool is_paused, is_vlc_time_in_milliseconds;
	...
	while (1)
	{
		...
		make_vlc_status_request(g_pNetworkAccessManager, g_pNetworkRequest, is_paused, cur_video_filename, is_vlc_time_in_milliseconds, video_pos, vlc_sys_time, cur_rate);
		{
			...
			tmp.is_valid = true;
			tmp.is_paused = is_paused;
			tmp.video_pos = video_pos;
			tmp.sys_time = vlc_sys_time;
			tmp.rate = cur_rate;
			tmp.video_filepath = cur_video_filename;
			get_cur_video_pos(tmp, cur_time, cur_video_pos);
		}
